...
When you examine the SQL Server Error log, you might notice that a significant amount of time is spent before the *analysis* phase of the database recovery process.
For example:
...
Additionally, SQL Server can log a [MSSQLSERVER_9017](errors-events/mssqlserver-9017-database-engine-error.md) error when you restore a database that has a large number of VLFs:
...
:::image type="content" source="media/sql-server-transaction-log-architecture-and-management-guide/transaction-log-after-truncate.png" alt-text="Diagram that illustrates how a physical log file is divided into virtual logs.":::
...
:::image type="content" source="media/sql-server-transaction-log-architecture-and-management-guide/transaction-log-wrap.png" alt-text="Diagram that illustrates how a logical transaction log wraps around in its physical log file.":::
...
- If the `FILEGROWTH` setting isn't enabled, or the disk that is holding the log file has less free space than the amount specified in *growth_increment*, a 9002 error is generated. For more information, see [Troubleshoot a full transaction log (SQL Server Error 9002)](logs/troubleshoot-a-full-transaction-log-sql-server-error-9002.md).
...
> [!IMPORTANT]  
> For more information about transaction log size management, see [Manage the size of the transaction log file](logs/manage-the-size-of-the-transaction-log-file.md).
...
The following diagrams show a transaction log before and after truncation. The first diagram shows a transaction log that has never been truncated. Currently, four virtual log files are in use by the logical log. The logical log starts at the front of the first virtual log file and ends at virtual log 4. The MinLSN record is in virtual log 3. Virtual log 1 and virtual log 2 contain only inactive log records. These records can be truncated. Virtual log 5 is still unused and isn't part of the current logical log.

:::image type="content" source="media/sql-server-transaction-log-architecture-and-management-guide/transaction-log-before-truncate.png" alt-text="Diagram that shows how a transaction log appears before it's truncated.":::

The second diagram shows how the log appears after being truncated. Virtual log 1 and virtual log 2 have been freed for reuse. The logical log now starts at the beginning of virtual log 3. Virtual log 5 is still unused, and it isn't part of the current logical log.

:::image type="content" source="media/sql-server-transaction-log-architecture-and-management-guide/transaction-log-after-truncate.png" alt-text="Diagram that shows how a transaction log appears after it's truncated.":::
...
The following diagram shows a simplified version of the end-of-a-transaction log with two active transactions. Checkpoint records have been compacted to a single record.

:::image type="content" source="media/sql-server-transaction-log-architecture-and-management-guide/active-log.png" alt-text="Diagram that illustrates an end-of-a-transaction log with two active transactions and a compacted checkpoint record.":::
